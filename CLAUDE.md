...
- **`sync/` (integrazione)** — `SyncController` è un'**interfaccia in `commonMain`** perché `AppContainer` e le schermate vivono lì mentre socket e crittografia stanno in `jvmSharedMain`: stessa ragione per cui `AlarmScheduler` è un'interfaccia. `SyncService` la implementa e tiene insieme ricerca, ascolto, associazione e giri di replica. Il parametro `listens` distingue i due lati: **il desktop ascolta sempre, Android mai** — le policy di sistema non lasciano tenere un socket aperto ad app chiusa, quindi il telefono sincronizza chiamando lui da `MainActivity.onStart()`. Ci si annuncia **solo se si è davvero in ascolto, e con la porta effettiva**: annunciarne un'altra manderebbe il peer contro un muro. Porta fissa `47653`, così il fallback manuale è indovinabile. Il flag `sync_enabled` (`AppSettings`) è **spento di default**: finché lo è, `start()` non apre socket e non annuncia nulla; si accende quando l'utente completa la prima associazione. I guasti di rete non risalgono come eccezioni ma diventano `SyncOutcome.Failed` con un messaggio in italiano.
