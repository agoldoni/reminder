...
- **`web/`** — porta HTTPS attivabile che espone ai browser della rete locale la vista principale, **in sola lettura**. Solo Android la cabla; il codice sta in `jvmSharedMain` (è `java.net` puro) con l'interfaccia `WebServerController` in `commonMain`, per la stessa ragione di `SyncController`. `AppContainer.web` ha un default **inerte** (`WebServerNonDisponibile`) così il desktop non deve conoscerla.
  ...
  **Vive quanto il processo, non quanto una schermata.** `WebService` è costruito in `ReminderApp.onCreate()`. Legarlo all'Activity vorrebbe dire rigenerare il token a ogni **rotazione dello schermo** — che è un giro completo di `onStop`/`onStart` — invalidando l'indirizzo appena digitato sull'altro dispositivo.
  **La porta resta aperta ad app chiusa**, tenuta viva da `WebServerService`, un servizio in primo piano. Non è ridondanza: un socket aperto in un processo in background è inutile, perché il processo viene congelato o ucciso e Doze taglia la rete. Quattro cose da sapere: (1) il tipo dichiarato è **`specialUse`** e non `dataSync`, perché da Android 15 quest'ultimo ha un tetto di sei ore al giorno — una porta che si chiuderebbe da sola a metà giornata; (2) `START_NOT_STICKY`, perché a processo ucciso il token ricomincerebbe diverso e la notifica dichiarerebbe raggiungibile un indirizzo che nessuno conosce; (3) dall'API 31 il sistema **rifiuta** `startForegroundService` da un'app che non è davanti, ed è per questo che `resume()` si chiama da `MainActivity.onStart()` e non da `ReminderApp.onCreate()` — un rifiuto non chiude la porta ma diventa un messaggio, perché finché l'app è aperta funziona lo stesso; (4) l'azione «Spegni» della notifica chiama `disable()` e quindi spegne **l'interruttore**, non solo il servizio: fermare il servizio lasciando acceso il flag farebbe riaprire la porta alla successiva apertura dell'app. La notifica non porta l'indirizzo: il token finirebbe sulla schermata di blocco.
  ...
  **Non è una PWA installabile sulla rete locale**, e la ragione non è più quella scritta qui prima. Il service worker non c'entra (Chrome non lo pretende) e nemmeno lo schema: con `https://` l'origine sarebbe un secure context. È il **certificato non fidato** a fermare l'installazione. Verificato: su `127.0.0.1` — via `adb forward` — Chrome offre «Installa», perché il loopback è comunque un contesto sicuro; sullo **stesso servizio raggiunto a un indirizzo di rete**, con l'avviso scavalcato, il pulsante «Installa» **non compare**. Sbloccarlo richiederebbe un certificato fidato, cioè la CA locale scartata dalla feature 003.
